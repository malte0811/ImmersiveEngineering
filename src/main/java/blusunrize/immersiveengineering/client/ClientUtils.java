	...
	public static List<BakedQuad>[] convertConnectionFromBlockstate(IExtendedBlockState s, TextureAtlasSprite t)
	{
		...
		for (Connection conn : conns)
		{
			...
			for (int i = 1; i < max&&i<f.length; i++)
			{
				...
				currList.add(new SmartLightingQuad(data, -1, EnumFacing.DOWN, pos));
				currList.add(new SmartLightingQuad(dataInv, -1, EnumFacing.UP, pos));
				...
				storeVertexData(data, 3, tmp, t, 16, 0, color, 255);
				storeVertexData(dataInv, 0, tmp, t, 16, 0, color, 255);
				currList.add(new SmartLightingQuad(data, -1, EnumFacing.WEST, pos));
				currList.add(new SmartLightingQuad(dataInv, -1, EnumFacing.EAST, pos));
